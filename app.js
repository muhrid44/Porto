...
Event.deleteMany({date: {$lt : new Date().toLocaleDateString("en-US")}}, function(err){
  if (err){
    console.log(err);
  }
});
...
participant.save(function(err){
  if (!err){
    Event.findOne({_id:req.body.id}, function(err, eventsFound){
      if (err){
        console.log(err);
      } else {
...
app.get("/delete/:id", function(req,res){
      ...
      Event.findByIdAndRemove({_id:deletedEvent}, function(err){
        if (err){
          console.log(err);
        } else {
          res.redirect("/event_manage");
  ...
  app.get("/edit/:id", function(req,res){
    ...
    Event.findOne({_id:editedEvent}, function(err, result){
      if (err){
        console.log(err);
      } else {
        const event_name = result.event_name;
  ...
  app.get("/admin_event/:id", function(req,res){
    ...
    Event.findOne({_id:idEvent}, function(err, eventFound){
      if (err) {
        console.log(err);
      } else {

        Participant.find({_id:eventFound.participant}, function (err, listFound){
          if (!err){
